...
import { PersonWithParentsPresented, PersonWithParentsResponse } from '@modules/persons/presenters/PersonWithParents.presenter'
...
export function usePerson({ projectId, personId }: UsePersonProps) {
  ...
  >({
    ...
    queryFn: async () => {
      ...
      if (response.status === StatusCode.OK && response.data) {
        ...
        const attributesThisPerson = attributes.filter(attr => attr.personId === personId)
  ...
  >({
    ...
    onSuccess: (_, { name, history, type, image, birthDate, deathDate, fatherId, motherId }) => {
        ...
        (cachedData: PersonQueryData) => {
          return {
            person: {
              ...
              history: history === undefined ? cachedData.person?.history : history,
              type: type === undefined ? cachedData.person?.type : type,
              image: image === undefined ? cachedData.person?.image : image,
              birthDate: birthDate === undefined ? cachedData.person?.birthDate : birthDate,
              deathDate: deathDate === undefined ? cachedData.person?.deathDate : deathDate,
              fatherId: fatherId === undefined ? cachedData.person?.fatherId : fatherId,
              motherId: motherId === undefined ? cachedData.person?.motherId : motherId,
            },
